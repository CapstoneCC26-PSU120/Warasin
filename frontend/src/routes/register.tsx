    ...
    <div className="min-h-screen bg-gradient-to-br from-[#e0f2fe] via-white to-[#fef08a] flex flex-col items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl p-8 border border-slate-100">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {error && <p className="text-red-500 text-sm text-center">{(error as any)?.response?.data?.message || 'Registration failed'}</p>}
          ...
          <p className="text-center text-sm text-slate-500 mt-4">
            Already have one? <Link to="/login" className="text-[#0284c7] hover:underline">Log in</Link>
          </p>
        </form>
      </div>
      ...
      <p className="mt-8 text-slate-500 text-sm">
        By continuing you agree to our calm and <span className="text-amber-500">friendly</span> terms.
      </p>
    </div>
  )
}
